...
const defaultMdLanguages = options.mdLanguages || ['en', 'zh']
                ...
                if (err) {
                    ...
                    if (annotationObject.attributes) {
                        for (let j = 0; j < annotationObject.attributes.length; j++) {
                            let attributeObject = { attribute: annotationObject.attributes[j], type: annotationObject.type }
                            const attributeAnnotationParseRes = parser.parseAnnotation(attributeObject)
                            delete attributeAnnotationParseRes.type
                            const splicingRes = writer.splicing(attributeAnnotationParseRes)
                            readRes = readRes.replace(`/// attributeTemplate<${templateName},${annotationObject.attributes[j].name}>`, splicingRes)
                        }
                    }
...
function walkMd() {
    ...
    for (let i = 0; i < defaultMdLanguages.length; i++) {
        const mdWalker = walk.walk((options.outPath || process.cwd()) + "/output" + `/${defaultMdLanguages[i]}`);
        ...
        mdWalker.on("file", function (root, fileStats, next) {
            if (fileStats.name.endsWith('.md')) {
                ...
                if (linkArray) {
                    createMdLink(root, root + "/" + fileStats.name, linkArray, readRes, defaultMdLanguages[i])
                }
                next();
            } else {
                next();
...
function walkYaml() {
    for (let i = 0; i < defaultMdLanguages.length; i++) {
        const yamlWalker = walk.walk((options.srcPath || process.cwd()) + "/yaml" + `/${defaultMdLanguages[i]}`);
        //遍历寻找文件，并读取
